...
mod test {
    ...
    pub fn test_deposit_into_escrow() {
        ...
        execute(|| {
            ...
            Ticketing::deposit_escrow(Origin::signed(alice.clone()), 100)
                .expect("Failed to deposit into escrow");
    ...
    pub fn test_deposit_into_penalty() {
        ...
        execute(|| {
            ...
            Ticketing::deposit_penalty(Origin::signed(alice.clone()), 100)
                .expect("Failed to deposit into penalty");
    ...
    pub fn test_unlock_deposits_and_withdraw() {
        ...
        execute(|| {
            ...
            Ticketing::unlock_deposits(Origin::signed(alice.clone()))
                .expect("Failed to unlock the deposit");
            // The unlock period is now set to UNLOCK_DURATION
            assert_eq!(<Deposits<Test>>::get(alice.clone()).unlock_at - Time::now(), <Timestamp<Test>>::from(UNLOCK_DURATION));
            ...
            Ticketing::withdraw(Origin::signed(alice.clone()))
                .expect("Failed to withdraw funds from deposit");
    ...
    pub fn test_unlock_deposit_twice() {
        ...
        execute(|| {
            ...
            Ticketing::unlock_deposits(Origin::signed(alice.clone()))
                .expect("Failed to unlock deposit");
            Ticketing::unlock_deposits(Origin::signed(alice.clone()))
    ...
    pub fn test_lock_deposit() {
        ...
        execute(|| {
            ...
            Ticketing::unlock_deposits(Origin::signed(alice.clone()))
                .expect("Failed to unlock deposit");
            Time::set_timestamp(2_000);
            Ticketing::lock_deposits(Origin::signed(alice.clone()))
                .expect("Failed to lock deposit");
    ...
    fn setup_deposit() {
        ...
        Ticketing::deposit_escrow(Origin::signed(alice.clone()), 150)
            .expect("Failed to deposit into the escrow");
        Ticketing::deposit_penalty(Origin::signed(alice.clone()), 50)
    ...
    fn test_redeem_ecdsa() {
        let keychain = <ecdsa::Pair as Pair>::from_string("//Alice", None)
            .expect("Could not create ECDSA Alice keychain pair");
    ...
    {
        test_redeem_with(sender, sender_pair, None, |sender, receiver|{
    ...
    fn test_redeem_insufficient_funds() {
        ...
        test_redeem_with(alice, keychain, None, |alice, bob|{
            Balance::make_free_balance_be(&alice, 1_000);
            Ticketing::deposit_escrow(Origin::signed(alice.clone()), 10)
                .expect("Could not deposit into escrow");
            Ticketing::deposit_penalty(Origin::signed(alice.clone()), 5)
                .expect("Could not deposit into penalty");
    ...
    fn test_redeem_funds_taken_from_penalty() {
        ...
        test_redeem_with(alice, keychain, None, |alice, bob| {
            Balance::make_free_balance_be(&alice, 1_000);
            Ticketing::deposit_escrow(Origin::signed(alice.clone()), 50)
                .expect("Could not deposit into escrow");
            Ticketing::deposit_penalty(Origin::signed(alice.clone()), 100)
                .expect("Could not deposit into penalty");
            Balance::make_free_balance_be(&bob, 1_000);
    ...
    fn test_redeem_losing_ticket() {
        ...
        test_redeem_with(alice, keychain, Some(U256::max_value() / 10), |alice, _| {
            Balance::make_free_balance_be(&alice, 1_000);
            Ticketing::deposit_escrow(Origin::signed(alice.clone()), 100)
                .expect("Couldn't deposit into escrow");
            Ticketing::deposit_penalty(Origin::signed(alice.clone()), 50)
                .expect("Couldn't deposit into penalty");
        }, |result, _, bob| {
    ...
    fn test_redeem_with<A, P, S, R>(sender: A, sender_pair: P, win_prob: Option<U256>, setup_function: S, result_handle: R) 
    where
        A: Into<AccountId32>,
        P: Pair,
        P::Signature: Into<MultiSignature>,
